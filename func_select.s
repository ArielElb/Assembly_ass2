...
run_func:
    ...
    pushq  %r13
    pushq  %r12
    movq   %rsi, %r15    #save the second string.
    movq   %rdi, %r14    #save the first string.



    subq   $31, %rdx   #subtract 31 from the input.
    cmpq   $6, %rdx    #check if the option is between 1-6.
    ja     .Invalid_Option    #if not, jump to the end of the function.
    jmp    *.Table(,%rdx,8) #jump to the right function.
