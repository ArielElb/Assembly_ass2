...
pstrijcpy:
    ...
    jg .Invalid_index   #if it is, jump to the invalid_index label.
    ...
    //TODO: for loop from i to j put attantion to the case i=j!!!
    jmp .for_pstrijcpy
...
.compare_pstrijcmp:
    ...
    #j in rcx
    movb    (%rsi,%rdx,1),%r13b   #copy the char from src to dst - r13b = src[i]
    movb    (%rdi,%rdx,1),%r12b   #copy the char from src to dst - dst[0+i] = r13b
    cmpb    %r13b,%r12b   #compare the chars
    incq    %rdx   #increase i
    je .for_pstrijcmp
    jg .src_bigger
    jmp .dst_bigger
